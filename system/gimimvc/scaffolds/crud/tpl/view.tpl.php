...
class View extends Scaffold
{
	...
	{
		$path =  SERVER_ROOT . DEFAULT_APPS_PATH . APP_PATH . DEFAULT_VIEW_PATH . DEFAULT_CONTENT_PATH . $this->name . '/';
		$this->file( $this->views, $path );
	}
...
<div class = 'row' >
	...
	<div class = 'span5 well' >
		<?php if ( isset( \$data['settings']['editing'] ) ): ?>
			<h1>Edit</h1>
			<hr>
			<br/>
			<p>
...
<div class =''>
	...
	<?php foreach( \$data['model']['data'] as \$row) :	?>
		<li class = 'span5'>
		<div class ='<?php if ( !isset ( \$data['settings']['show'] ) ) echo 'thumbnail'; ?> '>
			<div class ='caption'>
				{$this->thumbnails}
			<br/>
			<p>
			<?php if ( !isset ( \$data['settings']['show'] ) ): ?>
...
<table class ='table table-striped'>
	<tr>
		{$this->table_heads}
		<?php if ( !isset ( \$data['settings']['show'] )  || (  \$this->access()->permission ('u') || \$this->access()->permission ('d') ) ): ?>
		<th> Action </th>
		<?php endif; ?>
	</tr>
	<?php foreach( \$data['model']['data'] as \$row ) :	?>
...
<div class = 'row'>
	...
	<?php endif; ?>
	<hr/>
<?php	if( !empty( \$data['model']['data']) ):	?>
	<div class = 'span3'>
		<?php if ( !isset ( \$data['settings']['show'] ) && \$this->access()->permission('c') ): ?>
		<!-- <p><a class ='btn btn-primary btn-large' href='$name/form'><i class = 'icon-plus'></i> Add $uname</a></p> -->
		<?php elseif ( isset ( \$data['settings']['show'] ) ): ?>
...
<?php if ( isset ( \$data['model']['search'] ) ): ?>
<div class = 'well' >
...
VIEW;
		...
		/*********************** BEGIN HTML ****************************************************************/
		$gallery_bottom = <<<VIEW
	<?php echo Element::pager( \$data['model'] ); ?>
	<br/>
<?php else: ?>
	<div class = 'span2'>
		<br/>
...
<?php if ( !( !isset ( \$data['model']['search'] ) && empty( \$data['model']['data']) ) ): ?>
$search
<?php endif; ?>
VIEW;
		######################## END HTML   #################################################################
...
$this->inputs .= <<<input
	...
	<label>$uc_column: <?php echo html::$helper ('$column', \$data['model']['saved_fields']['$column'] ) ?> </label>
input;
######################## END HTML   #################################################################
/*********************** BEGIN HTML ****************************************************************/
$this->table_heads.= <<<heads
	<th>
	<a href='<?php
		echo '$name/gallery/' . \$data['model']['page'] .  VAR_SEPARATOR . '$column_id' . VAR_SEPARATOR;
		echo ( stristr( \$data['model']['order'], '$column_id/ASC' ) ) ? 'DESC' : 'ASC';
		echo \$data['model']['search'];
		?>'
	>
	$uc_column
	</a>
